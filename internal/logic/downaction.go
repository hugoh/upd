...
func (dal *DownActionLoop) Execute(execString string) error {
	...
	logger.Logger.WithField("exec", cmd.String()).Info("[DownAction] executing command")
	err := cmd.Start()
	if err != nil {
		logger.Logger.WithField("exec", cmd.String()).WithError(err).Error("[DownAction] failed to run")
		return fmt.Errorf("failed to execute DownAction: %w", err)
	}
	go func() {
		err := cmd.Wait()
		if err != nil {
			logger.Logger.WithField("exec", cmd.String()).WithError(err).Warn("[DownAction] error executing command")
		}
	}()
	return nil
}
...
func (dal *DownActionLoop) run(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			logger.Logger.Debug("[DownAction] canceled")
			return
		case <-time.After(dal.it.sleepTime):
		}
		err := dal.Execute(dal.da.Exec)
		if err != nil {
			logger.Logger.WithError(err).Error("[DownAction] failed to execute")
		}
		if dal.da.Every > 0 {
			dal.iterate()
		} else {
...
func (da *DownAction) Start() *DownActionLoop {
	...
	logger.Logger.Debug("[DownAction] kicking off run loop")
	go dal.run(ctx)
	return dal
}
...
func (dal *DownActionLoop) Stop() {
	...
	logger.Logger.Debug("[DownAction] sending shutdown signal")
	dal.cancelFunc()
}
